...
//vingtNeufMarker.bindPopup("<h6>Nous sommes au 29 rue d’Ulm!</h6><");
...
d3.csv("places.csv", data => {
  ...
  list.forEach(place => {
    ...
    if(place.latitude){
      ...
      //).bindPopup(`<h4><a href="${place.lien}">${place.nom}</a></h4>`).addTo(map);
...
$.getJSON("liste_des_kiosques_presse_a_paris.geojson", geodata => {
  L.geoJSON(geodata, {
    style() {
      return {
        color: "#ff00ff",
...
$("#card-header-text").html("<strong>Workshop rue d’Ulm</strong>");
